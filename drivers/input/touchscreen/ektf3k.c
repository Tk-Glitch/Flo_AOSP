...
static int firmware_update_header(struct i2c_client *client,
			unsigned char *firmware, unsigned int page_number);
...
static ssize_t
elan_iap_write(struct file *filp, const char *buff, size_t count, loff_t *offp)
{
	int ret;
	char *tmp;
...
static long
elan_iap_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static ssize_t
elan_show_status(struct device *dev, struct device_attribute *devattr,
		char *buf)
{
...
static ssize_t
update_firmware(struct device *dev, struct device_attribute *devattr,
		const char *buf, size_t count)
{
	...
	firmware_fp->f_pos = 0;
	for (pos = 0, page_number = 0; pos < MAX_FIRMWARE_SIZE; pos +=
	     FIRMWARE_PAGE_SIZE, page_number++) {
		if (firmware_fp->f_op->read(firmware_fp, firmware + pos,
		    FIRMWARE_PAGE_SIZE, &firmware_fp->f_pos) !=
		    FIRMWARE_PAGE_SIZE)
			break;
	}
	...
	if (RECOVERY || 1 > 0) {
		...
		do {
			ret = firmware_update_header(client, firmware,
				page_number);
		...
		for (i = 0; i < FINGER_NUM; i++) {
			...
			if (active || mTouchStatus[i]) {
				...
				if (active) {
					...
					x = x > ts->abs_x_max ? 0 :
							ts->abs_x_max - x;
					y = y > ts->abs_y_max ? ts->abs_y_max :
							y;
					touch_size = ((i & 0x01) ?
						buf[size_index[i]] :
							(buf[size_index[i]] >> 4))
							& 0x0F;
					pressure_size = touch_size << 4;
					input_report_abs(idev,
						ABS_MT_TOUCH_MAJOR, touch_size);
					input_report_abs(idev,
						ABS_MT_PRESSURE, pressure_size);
					input_report_abs(idev,
						ABS_MT_POSITION_X, y);
					input_report_abs(idev,
						ABS_MT_POSITION_Y, x);
...
static void elan_ktf3k_ts_report_data2(struct i2c_client *client, uint8_t *buf)
{
	struct elan_ktf3k_ts_data *ts = i2c_get_clientdata(client);
	struct input_dev *idev = ts->input_dev;
	uint16_t x, y, touch_size, pressure_size, active = 0;
	uint16_t fbits = 0, checksum = 0;
	uint8_t i, num, idx = IDX_FINGER;
...
static void elan_ktf3k_ts_work_func(struct work_struct *work)
{
	...
	switch (buf[0]) {
	case NORMAL_PKT:
		rc = elan_ktf3k_ts_recv_data(ts->client, buf + 4, 40);
		up(&pSem);
			...
			unsigned int length)
{
	...
	for (i = 0; i < length; i += ret) {
		ret = i2c_master_send(client, buf + i,
			length < SIZE_PER_PACKET ? length : SIZE_PER_PACKET);
		if (ret <= 0) {
			retry_times--;
			ret = 0;
		}
...
static int firmware_update_header(struct i2c_client *client,
			unsigned char *firmware, unsigned int pages_number)
{
	int ret, i;
	int sendCount, recvCount, write_times;
	...
	if (sendI2CPacket(client, packet_data, 1) < 0)
		goto fw_update_failed;
...
page_write_retry:
		...
		if ((sendCount = sendI2CPacket(client, cursor,
		...
		if ((recvCount = recvI2CPacket(client, packet_data,
	...
	wait_for_IRQ_Low(client, 500000);

	if (recvI2CPacket(client, packet_data, 4) < 0)
		goto fw_update_failed;

	touch_debug(DEBUG_INFO, "[elan] %s: hello packet %2x:%2X:%2x:%2x\n",
		__func__, packet_data[0], packet_data[1], packet_data[2],
			  packet_data[3]);
			...
			const struct i2c_device_id *id)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR,
			0, MAX_FINGER_SIZE, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE,
			0, MAX_FINGER_PRESSURE, 0, 0);
...
// end Wake Gestures
	...
	if (RECOVERY || check_fw_version(touch_firmware,
	    sizeof(touch_firmware), ts->fw_ver) > 0)
